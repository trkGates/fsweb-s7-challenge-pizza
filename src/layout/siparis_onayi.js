...
function SiparisOnayi() {
  ...
  useEffect(() => {
    axios.get(`https://6456b1042e41ccf16923222e.mockapi.io/pizzaSiparisleri/${id}`)
    .then(response => {
...
<div id='onay-container-2-içerisi'>
  <div>
<p>Boyut: <span className='onay-font-kalin'>{OnayDatası.SecilenBoyut}</span></p>
<p>Hamur: <span className='onay-font-kalin'>{OnayDatası.seçiliHamur}</span></p>
<p>Ek Malzemeler: <span className='onay-font-kalin'>{OnayDatası.eklenenUrunler}</span></p>
<p>Sipariş Notu: <span className='onay-font-kalin'>{OnayDatası.siparisNotu}</span></p>
<p>Sipariş Toplamı: <span className='onay-font-kalin'>{OnayDatası.ÜrünFiyatı}</span></p>
